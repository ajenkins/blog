...
My site was already pretty fast to begin with as a consequence of the tools I used to build
...
of the main sources of modern website slowness, which is why I chose to avoid them from the beginning.
...
up myself, I knew that this was a pretty standard feature in modern tools for making websites.

So I knew of one bottleneck making my website slow (unoptimized images), I knew there could be others.
...
to a new position for no apparent reason, forcing you to scroll through the page searching for where
you were.

Google actually has a bunch of different tools for measuring your site performance.
[You can see an overview of those options here](https://web.dev/vitals-tools/).
...
very easy to use – all you need to do is give it a URL.
...
Here are my results for that page before I made any optimizations to my site
...
my site is automatically pretty fast. For comparison, when I ran this test using
...
that YouTube video on the page (the issues caused by YouTube are marked with an asterisk*).
...
The other problems are legit though and are fully within my control.
...
so if you just want to see the final product,
...
page head, it blocks the rest of the page from rendering until the CSS file
...
do add up. It's also not _that_ big of a deal, because browsers cache CSS
...
amount of CSS (whatever that means), and I figured my 100-line file probably qualified as small enough.
...
But after seeing it come up in my PageSpeeds report, I reversed my stance.

I followed the docs to the letter, so I'm not going to rehash the steps here.
The only thing that tripped me up a little is that I had to move my CSS file.
...
with it turned on, it didn't seem to do anything. Then I found a setting in Netlify
...
The first error I was getting was `AssertionError: tag myImage not found`.
...
into a Nunjucks file that I had. That fixed that error, but then I got
...
After I got that working, I decided to go back and see if I could get the plugin to work with Markdown file.
If I couldn't, I would either need to find a new solution, or commit to writing all of my future blog posts
...
Although it feels a little sneaky that Google encourages you to use its own image format from its
performance analysis tool, WebP is actually pretty cool. Basically, without any noticeable loss
...
This was actually really confusing to me because when you search online for "Mac OS Safari WebP"
the first result is [an article from MacRumors](https://www.macrumors.com/2020/06/22/webp-safari-14/)
stating that Apple would be adding support for WebP images to Safari in Safari 14.
So then I check what version of Safari I'm running right now on my Mac. Version 14.0! Yay!

But when I try switching my images over to WebP, they don't load in Safari. That's weird.
Maybe _I'm_ doing something wrong. So then I try my site in Chrome, and the images load fine.
...
of the page loaded. Because I'm an **Experienced Web Developer**, I knew that
...
[The official docs on how to do that are here](https://developers.google.com/youtube/iframe_api_reference).
...
(Thanks, Graham!). It even has a simple configuration option for "lite" mode, which automatically
...
in Safari I need to click on the video twice to play, but my research online suggested that this
...
but not right now while Safari still doesn't support WebP as I discussed above.
...
to be "cool" and use my own self-hosted copies of the needed JS and CSS files rather than using
...
malicious script and potentially do bad things to the users of your website. It's a remote possibility,
but it's such an easy thing to mitigate and the exact steps were in the README, so I though I might
...
the plugin to look for them in there instead of the external CDN. But it didn't work. When I checked the
...
works until now. For the most part, Eleventy seems to just take all of the files I feed it in the input
...
What you're supposed to do for any folder where you just want Eleventy to blindly copy over all of the
contained files, regardless of file type, is use the `addPassthroughCopy` function.
